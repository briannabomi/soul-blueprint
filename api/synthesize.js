...
export default async function handler(req, res) {
  ...
  const { ikigai, lifePath, geneKey } = req.body;
  if (!lifePath || !geneKey || !ikigai) {
    return res.status(400).json({ error: "Missing required fields: ikigai, lifePath, geneKey" });
  }
  ...
  try {
    ...
    return res.status(200).json({ soulPurpose, ikigaiSynthesis });
  } catch (err) {
    if (err instanceof Anthropic.APIError) {
      console.error("[synthesize] Anthropic API error:", err.status, err.message);
      return res.status(502).json({ error: "Anthropic API error", status: err.status, detail: err.message });
    }
    console.error("[synthesize] Unexpected error:", err.message);
    return res.status(502).json({ error: "Synthesis failed", detail: err.message });
  }
}
